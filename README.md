...
* DNS сервер и сервера NGINX дублируются при помощи CARP.   Virtual IP каждой пары NGINX отдаётся DNS сервером по алгоритму round-robin.  При отказе одного из серверов запросы обрабатывает его пара.
  =======
